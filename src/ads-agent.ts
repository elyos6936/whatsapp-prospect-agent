...
import { ADS_SYSTEM_PROMPT } from "./ads-persona.js";
import { getAppSettings, getRecentAdsAgentMessages, type AgentMessage } from "./db.js";
import { ADS_TOOL_DEFINITIONS, executeAdsTool } from "./ads-tools.js";
...
export async function chatWithAdsAgent(_userMessage: string): Promise<string> {
  const client = getOpenAiClient();
  const history = getRecentAdsAgentMessages(50);
  ...
  while (rounds < MAX_TOOL_ROUNDS) {
    ...
    if (assistantMsg.tool_calls?.length) {
      ...
      for (const toolCall of assistantMsg.tool_calls) {
        ...
        let result: string;
        try {
          result = await executeAdsTool(toolCall.function.name, args);
        } catch (err) {
