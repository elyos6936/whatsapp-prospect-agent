...
8. TON = celui imposé par la consigne « TON DU FIL » (tutoiement ou vouvoiement) — jamais l'autre. Pas de prénom du contact à tout va.
...
export async function generateWhatsAppReply(userId: number, input: {
  ...
  /**
   * Textes campagne qui tranchent le tutoiement / vouvoiement (accroche validée,
   * playbook, tours de simulation) — consultés avant la mémoire.
   */
  ...
  // Ton du fil : d'abord ce qui a déjà été envoyé, sinon la campagne / la mémoire.
  const tone = resolveReplyTone({
    sentMessages: policyHistory
      .filter((m) => m.direction === "sortant")
      .map((m) => m.body),
    campaignTexts: [...(input.toneSources ?? []), input.automationContext],
  });
...
Écris UNIQUEMENT le message WhatsApp au ${inbound ? "client" : "prospect"} (1-2 phrases, ${toneLabel(tone)}, majuscule en tête). Pas d'analyse, pas de plan, pas de « Il vient de… ».${
  ...
  const buildMessages = (extraSystem?: string): OpenAI.Chat.Completions.ChatCompletionMessageParam[] => [
    ...
    { role: "system", content: toneInstruction(tone) },
